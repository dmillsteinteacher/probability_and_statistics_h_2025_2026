...
# Chart Formatting - Using simpler labels to prevent ParseErrors
ax.set_title(f"Visualizing Error and Power (n={n}, alpha={alpha})", fontsize=14)
...
m3.metric("Power (1-β)", f"{power:.3f}")
m4.metric("Standard Error", f"{se:.3f}")

# 7. Instructional Context
with st.expander("🎓 Interpret the Results"):
    st.write(f"""
    - **Type I Error (α):** There is a **{alpha*100:.0f}%** chance you will reject a true Null Hypothesis.
    ...
    **Try this:** Increase the Sample Size ($n
